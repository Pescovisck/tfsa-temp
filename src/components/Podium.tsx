...
export const Podium: React.FC<PodiumProps> = ({ standings }) => {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-3.5 sm:gap-5 mb-6 sm:mb-10 items-end">
      ...
      <div className="order-2 md:order-1 bg-white dark:bg-[#18181b] border border-slate-200/90 dark:border-zinc-800/90 rounded-2xl p-4 sm:p-5 relative overflow-hidden flex flex-col items-center text-center shadow-[0_4px_20px_rgba(15,23,42,0.05)] dark:shadow-[0_4px_20px_rgba(0,0,0,0.4)] hover:shadow-lg transition-all group">
        <div className="absolute top-0 inset-x-0 h-[3px] bg-gradient-to-r from-transparent via-slate-400 dark:via-zinc-600 to-transparent"></div>
        
        <div className="w-11 h-11 sm:w-13 sm:h-13 rounded-2xl bg-gradient-to-br from-slate-100 to-slate-200 dark:from-zinc-800 dark:to-zinc-900 border border-slate-300/80 dark:border-zinc-700 flex items-center justify-center text-slate-700 mb-2 sm:mb-3 shadow-sm group-hover:scale-105 transition-transform">
          <Medal className="w-5 h-5 sm:w-6 sm:h-6 text-slate-600 dark:text-zinc-300" />
        </div>
